# Hi, I'm Atabek 👋

English · [Türkçe](README.tr.md)

I build mobile apps and the backends behind them — Flutter and Dart on the front, Rust and Node.js on the back, PostgreSQL and Docker underneath. Six years of it, mostly in Web3, fintech, and logistics. I like owning a feature end to end: UI, API, database, deployment.

I use AI tools and local LLMs daily to move faster. Bronze medalist at the 2018 International Open Mathematics Olympiad — I still approach most problems the same way.

Open to remote and relocation roles.

💼 [LinkedIn](https://www.linkedin.com/in/atabek-kadirov) · ✉️ [Email](mailto:atabek.iuhd@gmail.com) · ✈️ [Telegram](https://t.me/ikaysin)

---

## ⭐ Featured projects

The ones shipped to production.

⏱️ [TeamTime](https://github.com/kaysman/ikay-production-portfolio/tree/main/teamtime) - Time tracking for Basecamp, done right. Live Basecamp integration with no extra accounts, 1–2 tap logging, and privacy-first storage of only the minimum data needed

🔢 [Raqam971](https://github.com/kaysman/ikay-production-portfolio/tree/main/raqam971) - UAE marketplace for buying and selling exclusive plate numbers and phone numbers

💳 [Digitax](https://github.com/kaysman/ikay-production-portfolio/tree/main/digitax) - Payment processing app built on Clean Architecture, with Stripe integration and offline-first data sync

🔐 [THXConnect Wallet](https://github.com/kaysman/ikay-production-portfolio/tree/main/thxconnect) - Web3 wallet and DAO platform with NFT management and Japanese market features (My Number)

🏃 [HealthyHour](https://github.com/kaysman/healthy_hour) - Wellness app for work-life balance: activity tracking, smart reminders for movement breaks, and health monitoring

More detail on each — stack, role, screenshots — in the [full portfolio](https://github.com/kaysman/ikay-production-portfolio).

---

## 🔧 Open source

🏃 [healthy_hour](https://github.com/kaysman/healthy_hour) - Wellness app for work-life balance: activity tracking, smart reminders for movement breaks, and health monitoring

📮 [api-catalog](https://github.com/kaysman/api-catalog) - Postman-style API console, generated from your Express, NestJS, and Rust routes. No spec files

🌍 [lsync](https://github.com/kaysman/lsync) - Rust CLI that syncs translations from Google Sheets straight into Flutter and React projects

📡 [modem-sms-listener](https://github.com/kaysman/modem-sms-listener) - Listens for incoming SMS on a USB GSM modem, parses the message payload, and publishes the location to a NATS server
